...
name: feishu-cli-msg
description: >-
  飞书消息发送与回复。发送消息（text/post/interactive 卡片等 11 种类型）、回复消息、
  转发/合并转发、发送图片/文件/卡片通知。当用户请求"发消息"、"回复"、"转发"、
  "发卡片"、"通知某人"、"给群里发一条消息"时使用。如果用户主要是想查看聊天记录、
  看群里最近说了什么、做 Pin/Reaction/删除消息、查群成员或管理群聊，请优先使用
  `feishu-cli-chat`。如果用户要按关键词搜索消息，请优先使用 `feishu-cli-search`。
