...
description: >-
  飞书综合工具箱：电子表格、日历日程、任务管理、画板操作、PlantUML 图表、文件管理、
  素材上传下载、文档评论、知识库、用户通讯录等综合能力。当用户请求操作飞书表格、
  查看日历、创建任务、操作画板、生成 PlantUML、管理文件、上传素材、查看评论、
  查看知识库、查询用户信息、查询部门时使用。如果用户主要是想登录飞书或排查 token，
  请优先使用 `feishu-cli-auth`；如果是搜索文档/消息/应用，请优先使用 `feishu-cli-search`；
  如果是查看聊天记录、群消息或群管理，请优先使用 `feishu-cli-chat`。
